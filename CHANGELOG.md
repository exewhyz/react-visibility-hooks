...
- Every release must include dated notes with sections: `Added`, `Changed`, `Fixed` (and `Removed` if applicable).
- Every bullet must describe user-facing impact and reference the hook/API affected.
- Any API or option changes must include migration guidance in README/docs in the same release.
- `Unreleased` must be updated before merge for user-visible changes.

## [Unreleased]

### Added

- Planned for `v3.1.0`: `useInactivityTimeout` option to customize activity event sources for product-specific idle detection.

### Changed

- Planned for `v3.1.0`: `useNetworkAwarePolling` docs and examples to cover practical retry/backoff patterns for offline-first flows.
